...
<ul class="selection">
	...
	<?php $attrItems = $this->productItem->getRefItems( 'attribute', null, 'config' )->uasort( $sortfcn )->groupBy( 'attribute.type' ) ?>

	<?php foreach( $this->get( 'attributeTypes', [] ) as $code => $attrType ) : ?>

		<?php if( !empty( $attributes = $attrItems->get( $code, [] ) ) ) : ?>
...
<ul class="selection">
	...
	<?php $attrItems = $this->productItem->getRefItems( 'attribute', null, 'custom' )->uasort( $sortfcn )->groupBy( 'attribute.type' ) ?>

	<?php foreach( $this->get( 'attributeTypes', [] ) as $code => $attrType ) : ?>
		<?php foreach( $attrItems->get( $code, [] ) as $id => $attribute ) : ?>
